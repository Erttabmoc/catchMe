...
window.addEventListener('DOMContentLoaded', function(){
    ...
    Ennemy.prototype.moveEnnemy = function() {
        if(this.position.x + this.speed.x > canvas.width-this.radius || this.position.x + this.speed.x < this.radius) {
            console.log('Hit !');
            ballsHitSound.play();
            this.speed.x = -this.speed.x;
        }
        if(this.position.y + this.speed.y > canvas.height-this.radius || this.position.y + this.speed.y < this.radius) {
            console.log('Hit !');
            ballsHitSound.play();
    ...
    function mouseMove(e){
        if (mouseStatement){
         player.position.x = e.pageX - canvas.offsetLeft;
         console.log(player.position.x)
         player.position.y = e.pageY - canvas.offsetTop;
         console.log(player.position.y)
        }
    }

    function mouseClicked(e){
        if (e.pageX < player.position.x + player.radius + canvas.offsetLeft 
            && e.pageX > player.position.x - player.radius + canvas.offsetLeft 
            && e.pageY < player.position.y + player.radius + canvas.offsetTop 
            && e.pageY > player.position.y - player.radius + canvas.offsetTop){
            player.position.x = e.pageX - canvas.offsetLeft - player.radius;
            player.position.y = e.pageY - canvas.offsetTop - player.radius;
            mouseStatement = true;
    ...
    function detection(){
        for (let i=0; i<ennemies.length; i++){
            let distance = Math.sqrt(Math.pow(player.position.x - ennemies[i].position.x, 2) + Math.pow(player.position.y - ennemies[i].position.y, 2));
            if (distance <= player.radius + ennemies[i].radius){
                console.log('Crash !')
                crashSound.play();
                lost = true;
                console.log('lost status')
            }
        };
    };
